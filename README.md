# Reasonix Computer Use 0.8.0-alpha.4

Reasonix 专用的 Windows Computer Use 插件。任务完成优先，低 token 为优化目标。

Alpha.4 已完成 QQ、QQ 音乐、Ollama Desktop 等真实应用验证，重点修复高 DPI
...
如果是推荐给完全不了解 Python 的人，仍建议使用自包含 ZIP。Git 安装现在只要求系统
...
1. SessionStart 返回 `computer_use=setup_required` 和缺失模块。
2. Agent 告知用户将下载依赖，用户确认一次。
3. Agent 调用 `computer_system(operation="setup", params={"confirmed":true})`。
4. 安装在后台运行，Agent 按 `poll_after_seconds` 调用 `setup_status`，展示当前阶段和最近日志。
...
Alpha.4 当前包含 47 项自动测试，覆盖动作 schema、物理坐标、revision、首次依赖安装、
...
0.8.0-alpha.4 只正式打包 Windows 10/11 x64。macOS 和 Linux 将在 Windows 版本稳定
