...
- UIA、OCR 与窗口视觉统一返回窗口内物理像素；执行点击时由插件转换为屏幕物理坐标，不需要 Agent 处理 DPI 或窗口原点。
...
- 表格定位必须从名称框或 UIA 选中项验证真实选区，不再直接伪造 `selected:true`。
...
- OCR 的 `o*` 引用可直接交给 `click_ref`，也可使用带完整文字的 `click_text`；禁止把 OCR 矩形改成 `click_point`
- 最近一次感知引用会在本机保存五分钟的短时运行缓存，Reasonix 重启 MCP 后会重新感知并恢复目标，不直接复用旧坐标
- UIA 元素返回 `focused` 与 `selected` 状态；表格使用 `select_cell`/`select_range`、名称框或定位功能，不猜测单元格像素位置
...
Alpha.12 当前包含 107 项自动测试，覆盖动作 schema、物理坐标、revision、首次依赖安装、
