# Reasonix Computer Use 0.8.0-alpha.7

Reasonix 专用的 Windows Computer Use 插件。任务完成优先，低 token 为优化目标。

Alpha.7 已完成 QQ、QQ 音乐、Ollama Desktop 等真实应用验证，重点修复浏览器地址导航、重复输入和高 DPI
...
Alpha.7 当前包含 57 项自动测试，覆盖动作 schema、物理坐标、revision、首次依赖安装、
...
0.8.0-alpha.7 只支持 Windows 10/11 x64。本次仅更新 Git 源码，未构建新的安装器；macOS 和 Linux 将在 Windows 版本稳定
