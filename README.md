# Reasonix Computer Use 0.8.0-alpha.10

Reasonix 专用的 Windows Computer Use 插件。任务完成优先，低 token 为优化目标。

Alpha.10 已完成 QQ、QQ 音乐、Ollama Desktop 等真实应用验证，重点修复 GUI 任务路由、系统应用发现和表格定位
...
- UIA 元素返回 `focused` 与 `selected` 状态；表格使用 `select_cell`、名称框或定位功能，不猜测单元格像素位置
...
Alpha.10 当前包含 67 项自动测试，覆盖动作 schema、物理坐标、revision、首次依赖安装、
...
0.8.0-alpha.10 只支持 Windows 10/11 x64。本次仅更新 Git 源码，未构建新的安装器；macOS 和 Linux 将在 Windows 版本稳定
