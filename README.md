...
Alpha.3 已完成 QQ、QQ 音乐、Ollama Desktop 等真实应用验证，重点修复高 DPI
...
| 使用者 | 推荐方式 | 需要自行准备的环境 |
|---|---|---|
| 普通 Windows 用户 | GitHub Release 自包含 ZIP | 无需 Python 或 pip |
| 已有 Python 的开发者 | Reasonix Desktop Git 安装 | Python 3.10+ 和四个 Python 依赖 |
| 插件开发者 | 克隆仓库后本地安装 | Python 3.10+、运行依赖和测试依赖 |

如果是推荐给不了解 Python 的人，请让对方使用自包含 ZIP，不建议走 Git 安装。
...
Git 安装只下载插件源码，Reasonix 不会执行仓库里的安装脚本，也不会自动运行 `pip`。
用户需要先准备 64 位 Python 3.10 或更高版本。推荐 Python 3.12：

```powershell
winget install --exact --id Python.Python.3.12
```

安装 Python 后关闭并重新打开终端，执行：
...
python -c "import pyautogui, PIL, comtypes, rapidocr_onnxruntime; print('环境正常')"
```

四个模块均导入成功后，打开 Reasonix Desktop 的 **设置 → 插件 → Git 仓库**，输入：
...
Alpha.3 当前包含 43 项自动测试，覆盖动作 schema、物理坐标、revision、
