...
10. 任一桌面工具返回 `setup_required` 时，先告知用户将下载依赖；确认后调用 `computer_system(operation="setup", params={"confirmed":true})`，再按 `poll_after_seconds` 轮询 `setup_status`。不得改用 Shell 执行 pip。
